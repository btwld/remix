...
schema: 1
presets:
  acme: acme/registry.yaml
```

The preset names are yours. A consuming project must select a preset your
registry provides — if a project is on `preset: acme`, every registry it uses
must offer `acme`.
...
Commit the `registry/` directory and push. Consumers then register it:
