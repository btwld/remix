...
| message | cause |
| --- | --- |
| `No registry-stable branch is published for conceptadev/remix.` | The official registry has not been promoted yet, so `init` has no commit to pin. Nothing was written; retry once it is published. |
