...
| **Preset** | One design language inside a registry — `vanilla` officially. A project picks exactly one. |
...
Omitting `--preset` selects `vanilla`. Use `--prefix` and `--ui-path` to
control naming and location:
...
Projects on `remix.yaml` schema 1 or 2 read from a frozen snapshot shipped
inside the CLI. Upgrading the CLI never rewrites them and never updates their
templates. Migration is explicit:

```shell
dart run remix_cli:remix registry migrate
dart run remix_cli:remix add button --diff
```

Migration changes configuration only. Your prefix, UI path, installed files,
and generated adapters are untouched. A legacy `default` preset is recorded as
`vanilla`; other preset names are unchanged. Add `--ref registry-v4` to pin a specific
release instead of the newest stable one.

Migration does not claim your installed source matches the new pin. That is
what the `--diff` on the second line is for.
...
- `remix.yaml` schema 3, `index.yaml` schema 1, and catalog schema 2 are the
  stable contract. The CLI supports reading schemas 1 and 2 of `remix.yaml` for
  existing projects.
