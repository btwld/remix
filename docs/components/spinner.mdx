...
class FortalSpinnerExample extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Row(
      spacing: 16,
      children: [
        RemixSpinner(style: fortalSpinnerStyle(size: FortalSpinnerSize.size2)),
        RemixSpinner(style: fortalSpinnerStyle(size: FortalSpinnerSize.size3)),
      ],
    );
  }
}
...
const RemixSpinner({
  ...
  StyleSpec<RemixSpinnerSpec>? styleSpec,
})
```
</CodeGroup>
...
#### `styleSpec` → `StyleSpec<RemixSpinnerSpec>?`
