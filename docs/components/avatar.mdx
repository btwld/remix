  ...
  Widget build(BuildContext context) {
    return Row(
      ...
      children: [
        ...
        RemixAvatar(
          label: 'AB',
          style: fortalAvatarStyle(variant: FortalAvatarVariant.soft),
        ),
        // Solid variant - Bold background
        RemixAvatar(
          label: 'CD',
          style: fortalAvatarStyle(variant: FortalAvatarVariant.solid),
        ),
      ],
    );
  }
...
<Info>
  See the [fortalAvatarStyle source code](https://github.com/btwld/remix/blob/main/packages/remix/lib/src/components/avatar/fortal_avatar_styles.dart) for all available options.
</Info>
...
const RemixAvatar({
  ...
  StyleSpec<RemixAvatarSpec>? styleSpec,
})
```
</CodeGroup>
...
#### `styleSpec` → `StyleSpec<RemixAvatarSpec>?`
