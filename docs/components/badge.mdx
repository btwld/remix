  ...
  Widget build(BuildContext context) {
    return Row(
      ...
      children: [
        RemixBadge(
          label: '5',
          style: fortalBadgeStyle(variant: FortalBadgeVariant.solid),
        ),
        RemixBadge(
          label: 'New',
          style: fortalBadgeStyle(variant: FortalBadgeVariant.soft),
        ),
      ],
    );
  }
...
const RemixBadge({
  ...
  StyleSpec<RemixBadgeSpec>? styleSpec,
})
```
</CodeGroup>
...
#### `styleSpec` → `StyleSpec<RemixBadgeSpec>?`
