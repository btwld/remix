...
class ProgressExample extends StatelessWidget {
  ...
  RemixProgressStyle get style {
    return RemixProgressStyle()
        .wrap(WidgetModifierConfig.clipRRect(borderRadius: BorderRadiusGeometryMix.circular(10)))
  ...
  Widget build(BuildContext context) {
    return Column(
      ...
      children: [
        RemixProgress(
          value: 0.6,
          style: fortalProgressStyle(variant: FortalProgressVariant.surface),
        ),
        RemixProgress(
          value: 0.4,
          style: fortalProgressStyle(variant: FortalProgressVariant.soft),
        ),
      ],
    );
  }
...
const RemixProgress({
  ...
  StyleSpec<RemixProgressSpec>? styleSpec,
})
```
</CodeGroup>
...
#### `styleSpec` → `StyleSpec<RemixProgressSpec>?`
