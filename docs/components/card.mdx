...
class FortalCardExample extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Column(
      ...
      children: [
        RemixCard(
          style: fortalCardStyle(variant: FortalCardVariant.classic),
        ),
        RemixCard(
          style: fortalCardStyle(variant: FortalCardVariant.surface),
        ),
      ],
    );
  }
...
const RemixCard({
  ...
  StyleSpec<RemixCardSpec>? styleSpec,
})
```
</CodeGroup>
...
#### `styleSpec` → `StyleSpec<RemixCardSpec>?`
