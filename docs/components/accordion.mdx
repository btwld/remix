...
class _AccordionExampleState extends State<AccordionExample> {
  ...
  RemixAccordionStyle get itemStyle {
    return RemixAccordionStyle()
        ...
        .wrap(WidgetModifierConfig.clipRRect(borderRadius: BorderRadiusGeometryMix.circular(8)))
        .paddingX(16)
        .paddingY(14)
        .borderRounded(8)
...
#### `onExpanded<T>(RemixAccordionStyle value)`

Applies a style while the item is expanded.

#### `onCollapsed<T>(RemixAccordionStyle value)`

Applies a style while the item is collapsed.

#### `onCanCollapse(RemixAccordionStyle value)`

Applies a style while the item can be collapsed.

#### `onCanExpand<T>(RemixAccordionStyle value)`
