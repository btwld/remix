    ...
    return RemixTabs(
      ...
      child: Column(
        children: [
          ...
          const RemixTabView(tabId: 'tab1', child: Text('Content 1')),
          const RemixTabView(tabId: 'tab2', child: Text('Content 2')),
        ],
      ),
    );
  }
...
Methods on `RemixTabBarStyle` configure the tab-list container.
...
Methods on `RemixTabStyle` configure each interactive tab item.
...
#### `onSelected(RemixTabStyle value)`
...
Methods on `RemixTabViewStyle` configure each tab panel.
...
#### `onSelected(RemixTabViewStyle value)`
