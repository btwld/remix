    ...
    return Row(
      ...
      children: [
        RemixToggle(
          ...
          label: 'Ghost',
          style: fortalToggleStyle(variant: FortalToggleVariant.ghost),
        ),
        RemixToggle(
          selected: _value,
          onChanged: (v) => setState(() => _value = v),
          label: 'Outline',
          style: fortalToggleStyle(variant: FortalToggleVariant.outline),
        ),
      ],
    );
  }
...
const RemixToggle({
  ...
  RemixToggleStyle style = const RemixToggleStyle.create(),
