...
class _FortalRadioExampleState extends State<FortalRadioExample> {
  ...
  Widget build(BuildContext context) {
    return RemixRadioGroup<String>(
      ...
      child: Column(
        children: [
          FortalRadio(value: 'soft', variant: FortalRadioVariant.soft),
          FortalRadio(value: 'surface'),
        ],
      ),
    );
  }
...
#### `call<T>({required T value, bool enabled = true, bool autofocus = false, bool toggleable = false, FocusNode? focusNode, MouseCursor? mouseCursor, bool enableFeedback = true})`
