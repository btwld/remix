...
class FortalDividerExample extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Column(
      spacing: 16,
      children: [
        RemixDivider(style: fortalDividerStyle(size: FortalDividerSize.size1)),
        RemixDivider(style: fortalDividerStyle(size: FortalDividerSize.size3)),
      ],
    );
  }
}
...
const RemixDivider({
  ...
  StyleSpec<RemixDividerSpec>? styleSpec,
})
```
</CodeGroup>
...
#### `styleSpec` → `StyleSpec<RemixDividerSpec>?`
