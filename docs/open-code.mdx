...
The standalone Carbon registry's `stable` ref has not yet been promoted, so
its bare `--preset carbon` command is not available to consumers yet. Fortal
preset does not add a `remix_fortal` dependency.
</Info>
