...
=======
  add       Install one registry item and its dependencies.
  init      Initialize Remix source installation settings.
  registry  Register, pin, or migrate GitHub registry sources.
>>>>>>> 7afd6fa46 (feat(registry): publish a pinned GitHub registry and the Vanilla preset)
...
| Key | Default | Rules |
|---|---|---|
| `schema` | `3` | New files pin remote registries. Schemas 1–2 retain the frozen bundled snapshot until explicitly migrated. |
...
```bash
remix registry update @remix --ref registry-v2
remix add button --diff
```
...
Schema-1/2 projects retain their frozen bundle. Migrate configuration only with
`remix registry migrate --ref registry-v1`, then review with `remix add button --diff`.
Migration preserves prefix, preset, UI path, authored files and generated adapters.
Restore a previously committed `remix.yaml` pin to roll back the source selection;
installed files are not reverted.
...
never substitute bundled or newer content. Private repositories, arbitrary HTTP
