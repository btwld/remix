...
small `vanilla` preset, the full Radix Themes-inspired `fortal` preset, or the
initial IBM Carbon-inspired `carbon` slice when you initialize the project.
...
<Info>
The hosted installation commands below require `remix_cli 0.0.1-beta.1` on
pub.dev. Until it is available, use the checkout installation described in
the CLI README. The CLI has its own version and release schedule. Bare
`--preset carbon` additionally requires the standalone Carbon registry's
`stable` ref to be promoted. Fortal does not add a `remix_fortal` dependency.
</Info>

## Choose a preset

| | `vanilla` | `fortal` | `carbon` |
|---|---|---|---|
| Design language | Compact neutral starting point | Radix Themes 3.3.0 parity surface | IBM Carbon foundation |
| Theme | 15 semantic tokens | 277 tokens, Radix color scales, scaling, radius, and panel modes | White, Gray 10, Gray 90, and Gray 100 token maps |
| Component axes | Small default vocabulary | Fortal's `classic`, `solid`, `soft`, `surface`, `outline`, and `ghost` vocabulary where applicable | Carbon button kinds and sizes |
| Best for | A house system you intend to shape | Starting from a complete Radix-like system | Adopting the first Carbon theme-and-button slice |

All three choices install local Dart under `lib/`, use Remix for behavior, and use
...
installation below. All three presets require Remix `^1.0.0-beta.10`.
...
Prefer the project-local form to pin the CLI. Registry content is pinned
...
| `preset` | `vanilla` | Preset selected across every configured registry. Choose `vanilla`, `fortal`, or `carbon` at `init`. |
| `defaultRegistry` | `@remix` for Vanilla/Fortal; `@carbon` for Carbon | Configured namespace for bare item names. |
...
Carbon begins as a deliberately narrow standalone registry: its complete
theme foundation and `button`. The CLI selects `@carbon` at
`btwld/flutter-carbon` when you initialize; it pins that source's commit before
installing Button:
...
The application receives `AcmeScope`, `AcmeTheme`, `AcmeButton`, and
`acmeButtonStyle` as editable local source. `AcmeTheme.white`, `.g10`, `.g90`,
and `.g100` select Carbon's four themes. Button supports Carbon's primary,
secondary, tertiary, ghost, and danger treatments; its default 48 px height can
be overridden directly or inherited from `AcmeLayoutScope`.

This preset intentionally does not claim catalog parity yet. It contains no
icon button, chart, icons, Agent surface, or dashboard item. Its authored
source, generated distribution, and release gate live in the separate
[`btwld/flutter-carbon`](https://github.com/btwld/flutter-carbon) repository;
the CLI does not bundle a Carbon catalog.
...
The Vanilla and Fortal generated scopes expose `theme`, `darkTheme`, and `mode`. A root without
...
in the Vanilla and Fortal presets. Their generated theme files are editable application source;
...
Registry content releases independently of the CLI. Update a pin explicitly,
then review installed source; no automatic merge or installed-source lockfile
is used.
...
Bare dependencies resolve within their owning registry. Qualified dependencies
require explicit registration; they cannot add or redirect sources. Cross-registry
cycles, conflicting targets and incompatible package requirements fail before writes.
...
Upgrading the CLI does not change the pin, so the notice names
...
The separate `carbon` catalog is intentionally smaller: it contains `theme`
and `button` only. Adding Button installs both, generates `AcmeButton`, and
exposes the four Carbon theme maps through `AcmeScope`. Carbon does not inherit
the official Vanilla/Fortal catalog merely because item names overlap.
...
## Agent surfaces in Vanilla and Fortal
...
Agent recipes extend Vanilla and Fortal; they are not included in the Carbon
slice and do not claim Radix component parity.
