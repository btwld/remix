...
// =============================================================================
// PUBLIC BUILDER WRAPPER (keeps existing API)
// =============================================================================
class RadixTokenBuilder {
  static RadixResolvedColors build(RadixTheme theme) {
    return _buildResolvedColors(theme);
  }
}
  ...
  switch (color) {
    ...
    case RadixAccentColor.red:
      return RadixColorScale(
        ...
        isDark ? RadixColors.redDark.alphaSwatch : RadixColors.red.alphaSwatch,
      );
    case RadixAccentColor.sky:
      return RadixColorScale(
        isDark ? RadixColors.skyDark.swatch : RadixColors.sky.swatch,
        isDark ? RadixColors.skyDark.alphaSwatch : RadixColors.sky.alphaSwatch,
      );
...
Color _getContrastColor(RadixAccentColor accent) {
  ...
  return switch (accent) {
    ...
    RadixAccentColor.lime => RadixColors.olive.swatch[12]!, // close to #1D211C
...
Color _generateAccentSurface(RadixColorScale accent, Color surface) {
...
RadixResolvedColors _buildResolvedColors(RadixTheme theme) {
  // Get color scales
  ...
  final colorBackground = gray.step(1); // var(--gray-1)

  final colorPanelSolid = gray.step(2); // var(--gray-2)

  // Panel translucent uses specific values
  final colorPanelTranslucent = gray.alphaStep(2); // grayAlpha[a2]
