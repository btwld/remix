...
/// `UiDisclosure`: `triggerBuilder` and `transitionBuilder`. Both
...
DisclosureStyler uiDisclosureStyle({
  DisclosureStyler style = const DisclosureStyler.create(),
    ...
    .onHovered(DisclosureStyler().color(UiTokens.accent()))
    ...
    .onExpanded(DisclosureStyler().color(UiTokens.muted()))
...
DisclosureStyler _focusVisibleStyle() => DisclosureStyler().trigger(
  .foregroundDecoration(
    BoxDecorationMix.border(
      .color(UiTokens.focusRing()).width(_focusRingWidth),
    ).borderRadius(.all(UiTokens.radius())),
  ),
);
